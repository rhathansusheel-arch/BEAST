...
## MT5 session drop and config blockers, XAUUSD-only (2026-09-14)

The alerts that prompted this (`API_LOST` at 18:36:21 / 18:36:58 / 18:37:47,
`CONFIG_BLOCKER` with 19 keys) came from three local `--once --dry-run`
starts, each paging the startup reconcile once; the VPS bridge was healthy
throughout and `beast.service` was - correctly - refused by preflight because
demo account 5055782131 no longer exists at MetaQuotes. Nothing was flapping.
What the alerts *did* expose is below. The operator's standing instruction for
this task: **Beast trades XAUUSD only.**

| # | Decision | Why |
|---|---|---|
| D-79 | Blockers are scoped to the active markets. `Config.BLOCKER_SCOPE` names the instrument or family each key gates; `unset_blockers(markets)` returns only the keys that gate one of `broker.symbols`, and `main.py` builds only the adapters those markets route to (`_build_brokers`) - an XAUUSD-only agent never instantiates Zerodha and is never blocked by, or paged about, `instruments.nifty.*`, `instruments.sensex.*` or `options.*`. `--check` still prints the whole-config view, marked inactive. The Nifty/Sensex lot sizes and the `options.*` floors are **not** filled: they never had values in this repo - the numbers seen earlier were the test fixture in `tests/conftest.py`, not a deployment that regressed. | The CONFIG_BLOCKER alert listed seven keys for markets Beast does not trade. Filling them to silence it would put unreviewed numbers in a config for a leg that is switched off. |
| D-80 | The MT5 bridge is retried from the trading loop with capped exponential backoff: `MT5Adapter.maintain()` runs every cycle, schedules the first attempt `retry.reconnect_base_seconds` after a drop, doubles to `retry.reconnect_max_seconds`, and jumps to the ceiling at once when the connection layer marked the login misconfigured (`-6`). Each attempt is one handshake (`reconnect(wait_seconds=0)`), bounded by the `connect` timeout class, so the exit path is never blocked by a retry window. `API_LOST` pages once on the drop and again every `ops.api_lost_reminder_minutes`; the return is `API_RESTORED` and triggers `_after_broker_restored`: gold specs re-validated, the routed markets without an in-process position reconciled against the venue, the startup pause lifted, the blocker board refreshed. The blocker alert itself moved from before broker discovery to after it and is edge-triggered on the list. "System online" now says **ready** or **NOT READY TO TRADE** with the reasons, carried in `heartbeat.json` (`system.ready`, `system.readiness_issues`) and shown on the dashboard. | `MT5Connection.reconnect()` existed and nothing called it: a HALTED bridge stayed halted until a restart. The blocker alert fired for gold keys the venue supplies seconds later, so every correct start read as broken. |
| D-81 | Gold specs are cached to `ops.gold_specs_cache_path`, keyed by login and server, written on every successful validation and re-read at startup, on every reconnect and every `ops.gold_specs_revalidate_hours`. A value that differs from the cache or from the first read this session is `SPEC_DRIFT` (critical, rate-limit exempt): logged with old and new, **not applied**, and entries on the market pause until the operator confirms the change at the venue, deletes the cache and restarts. A cache from another account is ignored, not compared. A connected venue with no usable symbol is `SYMBOL_UNRESOLVED`, distinct from `API_LOST`. `instruments.gold.venue` stays `<company> / <server>` (D-56) rather than the literal `MT5` the brief suggested - the routing already says MT5; the venue string says *which* MT5. `account_currency` is still the operator's declaration (D-58) and is never read from the venue: reading it would make sizing silently assume `risk.capital` is in whatever currency the account happens to be in. Preflight runs the blocker check after the symbol check and excuses exactly the keys discovery will fill, never `account_currency`. | Drift in `contract_multiplier` or `tick_value` is a wrong position size with no error anywhere; the only defence is comparing against a value that survives a restart. Preflight refusing every correctly configured start on keys the venue fills was the other half of "nothing executes". |
| D-82 | `data.gold_spread_max` is auto-calibrated while `null` and `data.gold_spread_auto_calibrate` is on: `data/spread_calibration.py` samples a burst of quotes per cycle while the gold session is open, and when `gold_spread_calibration_seconds` have elapsed sets the ceiling to `p90 x gold_spread_safety_multiplier` (2.5, inside the brief's 2-3x), logging every input. A number in the config always wins and ends the auto mode. Every threshold that is auto-calibrated or listed under `risk.unreviewed_thresholds` is printed in a boot banner, raised as `THRESHOLDS_UNREVIEWED`, carried in the heartbeat - and **`mode: live` is refused by `_step_config` while one gates an active market**. `options.min_*` get no placeholders: the Indian leg is inactive (D-79). | The brief's own words: a spread ceiling is the operator's tolerance and "getting this wrong either blocks everything or blocks nothing". A banner makes a placeholder visible; the refusal makes it impossible to carry one into live capital by forgetting. |
| D-83 | The watchdog's "Beast exited deliberately (code N); not restarting" page is sent once per exit and then every `ops.watchdog_reminder_minutes`, not once per 15-second pass. | The 2026-09-14 alert log holds 125 identical CIRCUIT_BREAKER pages for one preflight refusal between 12:44 and 13:14 - the same fatigue as the API_LOST storm, one layer up. A critical kind bypasses the rate limiter by design, so the sender has to be edge-triggered itself. |

Still blocked, and not by code: every gold value in this table is derived from
a live MT5 session, and demo account 5055782131 was deleted server-side on
2026-09-11 (`-6, Terminal: Authorization failed`, verified again today). No
symbol, spec, spread or fill can be observed until a working demo login is
supplied. `instruments.gold.account_currency` and `risk.capital` (500000 - a
rupee figure under D-41) are the two risk-sizing inputs that remain the
operator's to set for a USD gold account; neither is guessed here.
